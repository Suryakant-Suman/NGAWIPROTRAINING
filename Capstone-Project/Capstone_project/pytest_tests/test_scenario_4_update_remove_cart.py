...
def test_scenario_4_update_remove_cart(driver, account_page, shared_users, row):
    ...
    cart_page.update_quantity(int(row["quantity"]))
    cart_page.remove_product(row["product"])
    cart_page.assert_product_not_present(row["product"])
