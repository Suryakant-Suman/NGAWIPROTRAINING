...
def shared_users(browser_name):
    ...
    log_step(f"[Setup] Creating {len(data)} shared users for run {run_id}")

    for i, row in enumerate(data, start=1):
        email = f"qa_user{i}_{run_id}@example.com"
        password = row["password"]
        page.register_user(row["firstname"], row["lastname"], email, row["phone"], password)
        page.logout_user()
