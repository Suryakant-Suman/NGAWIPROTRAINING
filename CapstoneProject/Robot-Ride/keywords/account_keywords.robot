...
Login User With Shared Registration
    ...
    ${user_index}=    Get Current Scenario User Index
    Ensure Shared User Pool Has Index    ${user_index}    ${firstname}    ${lastname}    ${phone}    ${password}
    ${user_number}=   Evaluate    ${user_index} + 1
    ${email}=         Get From List    ${SHARED_USER_EMAILS}       ${user_index}
    ${password}=      Get From List    ${SHARED_USER_PASSWORDS}    ${user_index}
    Log Operation    [Login] Using shared user index ${user_number}: ${email}
    Login User With Credentials    ${email}    ${password}
